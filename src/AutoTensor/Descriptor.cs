...
namespace AutoTensor
{
	...
	public class Descriptor
	{
		...
		public (Tensor<double> X, Tensor<double> Y) ToExamples(IEnumerable<object> examples)
		{
		...
		public Tensor<double> ToMatrix(IEnumerable<object> examples)
		{
		...
		public Tensor<double> ToVector(object item)
		{
		...
		public static Descriptor Create(Type t)
		{
			...
			{
				var item = property.GetCustomAttributes(typeof(AutoTensorAttribute), false);

				if (item.Count() == 1)
				{
					var attrib = (AutoTensorAttribute)item.First();
